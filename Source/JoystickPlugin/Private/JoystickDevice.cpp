...
void FJoystickDevice::JoystickButton(FDeviceId DeviceId, int32 Button, bool Pressed)
{
	...
	if (DeviceSDL->GetDevice(DeviceId)->Name.Contains(TEXT("Valleg")))
	{
		//발레그 Generic input 통합
		FString InitKeyName = TEXT("Gamepad_");
		switch (Button)
		{
		case 8:
			InitKeyName.Append(TEXT("LeftTrigger"));
			break;
		default:
			break;
		}
		FKey GenericKey = FKey(*InitKeyName);
		FKeyEvent KeyEvent(GenericKey, FSlateApplication::Get().GetModifierKeys(), 0,0, 0, 0);
		if (Pressed)
...
{
	if (FMath::IsNearlyEqual(CurrentState[DeviceId].Axes[Axis], Value, 0.01f) == false) {
		...
		if (DeviceSDL->GetDevice(DeviceId)->Name.Contains(TEXT("Valleg")))
		{
			...
			{
			case 0:
				InitKeyName.Append(TEXT("LeftX"));
				break;
			case 1:
				InitKeyName.Append(TEXT("LeftY"));
				break;
			case 4:
				InitKeyName.Append(TEXT("RightY"));
				break;
			default:
				break;
			}
			FKey GenericKey = FKey(*InitKeyName);
			FAnalogInputEvent GenericAnalogInputEvent(GenericKey, FSlateApplication::Get().GetModifierKeys(), 0, false, 0, 0, Value);
			FSlateApplication::Get().ProcessAnalogInputEvent(GenericAnalogInputEvent);
		}
...
void FJoystickDevice::SendControllerEvents()
{
	...
	if (inputsetting)
	{
		for (auto& setting : inputsetting->AxisConfig)
		{
			if (setting.AxisKeyName == TEXT("Gamepad_RightY"))
			{
				setting.AxisProperties.DeadZone = 0;
			}
		}
	}
